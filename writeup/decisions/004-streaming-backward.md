...
dQ is local, no atomics; each q_block writes its own row range. dK/dV pays one atomic_add per (q_block, kv_block) hit, roughly `top_k * n_q_blocks` atomics per (batch, head). On H100 this is well under bandwidth and the bottleneck is the softmax recomputation. The end-of-kernel cast absorbs the precision step where fp32 streaming math is already running. Correctness: `max_abs_err < 5e-3` against an autograd-through-reference gradcheck at the headline shape.
